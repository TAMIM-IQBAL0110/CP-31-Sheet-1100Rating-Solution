...
void solve(){
    ll n,m,k; cin>>n;
    vector<ll>v(n),a,cs(n);
    vecinp(v,n);
    a = v;
    asort(a);
    cs = a;
    for(int i=1;i<n;i++)cs[i]+=cs[i-1];
    // for(auto it:cs) cout<<it<<' ';cout<<endl;
    vector<int>lastReach(n);
    map<int,int>mp;
    for(int i=n-1;i>=0;i--){
        int pos = upper_bound(a.begin(),a.end(),cs[i])-a.begin();
        // cout<<pos<<endl;
        lastReach[i] = max(lastReach[pos-1],pos-1);
        mp[a[i]] = max(mp[a[i]],lastReach[i]);
    }
    for(int i=0;i<n;i++){
        cout<<mp[v[i]]<<' ';
    }
