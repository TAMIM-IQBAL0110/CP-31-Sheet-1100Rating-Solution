...
void solve(){
    ...
    ll ans = llabs(sr.first-des.first)+llabs(sr.second-des.second);
    ll cntd = 1e16,cntsr = 1e16;
    for(int i=0;i<k;i++){
        ll temp = llabs(v[i].first-sr.first)+llabs(v[i].second-sr.second);
        cntsr = min(cntsr,temp);
    }
    for(int i=0;i<k;i++){
        ll temp = llabs(v[i].first-des.first)+llabs(v[i].second-des.second);
        cntd = min(cntd,temp);
    }
    // cout<<cntsr<<' '<<cntd<<endl;
    ans = min(ans,cntsr+cntd);
    // }
    cout<<ans<<endl;
