...
vector<int>alldiv(int n){ // complexity :O(sqrt(n))
    ...
    for(int i=1;i*i<=n;i++){
        if(n%i==0){
            bool pwr2 = ((i & (i-1)) == 0);
            bool pwr2_2 = (((n/i) & ((n/i)-1)) == 0);
            if(n/i==i && pwr2) div.push_back(i);
            else {
                if(pwr2) div.push_back(i);
                if(pwr2_2) div.push_back(n/i);
            }
        }
    }
    return div;
...
void solve(){
    ll n,m,k; cin>>n>>k;
    vector<ll>v(n);
    vector<int>b,fre(31,0);
    vecinp(v,n);
    for(int i=0;i<k;i++){
        int x; cin>>x;
        if(fre[x] == 0) b.push_back(x),fre[x] = 1;
    }
    for(int i=0;i<n;i++){
        for(int j = 0;j<b.size();j++){
            ll temp  = 1<<b[j];
            if(v[i]%temp == 0) v[i]+=(temp/2);
        }
    }
