...
void solve(){
    ...
    vecinp(v,n);
    map<ll,ll>mx,mn;
    vector<int>divs = alldiv(n);
    // for(auto it:divs) cout<<it<<' ';cout<<endl;
    for(int i=1;i<n;i++) v[i]+=v[i-1];
    for(int i=0;i<n;i++){
        int idx = i+1;
        for(int j = 0;j<divs.size();j++){
            if(divs[j]<=idx && idx%divs[j] == 0){
                if(divs[j]<idx){
                   ll por = v[i] - v[i-divs[j]];
                //    cout<<divs[j]<<' '<<por<<endl;
                   mx[divs[j]] = max(mx[divs[j]],por);
                   mn[divs[j]] = min(mn[divs[j]],por);
                }
                else{
                   mx[divs[j]] = v[i];
                   mn[divs[j]] = v[i];
    ...
    // for(auto it:mn) cout<<it.first<<' '<<it.second<<endl;
    ll ans = 0;
    for(int i=0;i<divs.size();i++){
        ll dif = mx[divs[i]]-mn[divs[i]];
        ans = max(dif,ans);
    }
    cout<<ans<<endl;
